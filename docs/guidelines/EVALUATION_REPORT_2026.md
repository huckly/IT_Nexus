...
| **已實現** | 序號、型號、製造商、**全量介面 (Port)**、**全量 IP 位址**、**Inventory (PSU/Fan)** 全自動同步 (v6.0 新增) |

> [!NOTE]
> **v6.0 進展：介面與資產補完**  
> 已實現自動同步交換器所有實體 Port 與 IP 綁定，並補足了電源與風扇等硬體組件清單。
...
|------|------|
| **滿足程度** | ⭐⭐⭐⭐ (80%) |
| **已實現** | 設備上下線狀態變更 (Active ↔ Decommissioning) 會即時反映至 NetBox (透過 Webhook) |
| **已實現** | 同步週期缩短至 **15 分鐘** (Systemd Timer)，並支援 **Webhook 即時觸發** |
| **已實現** | 同步失敗時會透過 Email 通知 (`sync-failure-notify@.service`) |

> [!NOTE]
> **v6.0 進展：實現「第一時間」同步**  
> 透過修復 Webhook 接收端並部署高頻 Timer，異動延遲從 24 小時縮短至數分鐘內。
